...
- **Deploy automation:** `scripts/deploy/` — EC2 backend (`connitor.bengalurutechcommunity.com`) + Amplify frontend zip/Git. Copy `config.example.env` → `config.env`, then `.\scripts\deploy\deploy-all.ps1`. Guide: `docs/knowledge/deployment/DEPLOY-AUTOMATION.md`.
