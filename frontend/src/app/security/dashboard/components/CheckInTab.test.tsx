...
describe('CheckInTab', () => {
  ...
  const renderCheckInTab = (props: { branchId: string; onCheckInSuccess?: (visitId: string) => void }) => {
    return render(<CheckInTab key={testKey} {...props} />);
  };
  ...
  describe('Rendering', () => {
    ...
    it('should render OTP input description', () => {
      renderCheckInTab({ branchId: mockBranchId });
      expect(
        screen.getByText("Enter the 6-digit OTP from the visitor's gate pass")
      ).toBeInTheDocument();
    });
    ...
    it('should render phone lookup button', () => {
      ...
      expect(screen.getByText('Check Visitor')).toBeInTheDocument();
    });
    ...
    it('should render instruction items', () => {
      renderCheckInTab({ branchId: mockBranchId });
      expect(screen.getByText(/Ask the visitor for their 6-digit OTP/i)).toBeInTheDocument();
      expect(screen.getByText(/Enter the OTP above and click Verify/i)).toBeInTheDocument();
      expect(screen.getByText(/Verify visitor identity matches the displayed details/i)).toBeInTheDocument();
      expect(screen.getByText(/Click Check In to complete the process/i)).toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    it('should have role="tabpanel"', () => {
      render(<CheckInTab branchId={mockBranchId} />);
      const tab = screen.getByTestId('check-in-tab');
  ...
  describe('OTP Verification - Happy Path', () => {
    ...
    it('should trigger onCheckInSuccess callback when Check In is clicked', async () => {
      ...
      render(<CheckInTab branchId={mockBranchId} onCheckInSuccess={mockOnCheckInSuccess} />);
  ...
  describe('Phone Lookup Navigation', () => {
    ...
    it('should navigate back to OTP view from phone lookup', async () => {
      render(<CheckInTab branchId={mockBranchId} />);
      ...
      await waitFor(() => {
        expect(screen.getByText('Enter the 6-digit OTP from the visitor\'s gate pass')).toBeInTheDocument();
      });
    });
  });
