    ...
    try {
      if (isEditMode && userToEdit) {
        ...
        if (result.credentialsEmailSent) {
          ...
          toast.warning(
            result.emailWarning ?? 'User created, but the credentials email could not be sent.',
          );
        }
      }
      onSaved();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Create or update users across departments in your hospital.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
...
export function HospitalAdminUser({ user }: Props) {
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState('deptAdmins');
  const [userToEdit, setUserToEdit] = useState<User | null>(null);
  const [userToDeactivate, setUserToDeactivate] = useState<User | null>(null);

  const { data: users, isLoading, mutate } = useSWR<User[]>(
    `/api/users/branch-${user.branchId}`,
    () => UserService.getAll({ branchId: user.branchId }),
  );
  const { data: departments } = useSWR<Department[]>(
    `/api/departments/branch-${user.branchId}`,
    () => DepartmentService.list({ branchId: user.branchId }),
  );
  const { data: subDepartments } = useSWR<SubDepartment[]>(
    `/api/sub-departments/branch-${user.branchId}`,
    () => SubDepartmentService.list({ branchId: user.branchId }),
  );
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Users className="h-6 w-6" /> Hospital User Management
          </h1>
          <p className="text-muted-foreground">Manage all users at your hospital site</p>
        </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          <TabsTrigger value="deptAdmins">Dept Admins</TabsTrigger>
        ...
        <TabsContent value="deptAdmins">
          <UserTable users={deptAdmins} isLoading={isLoading} onEdit={(u) => { setUserToEdit(u); setOpen(true); }} onDeactivate={setUserToDeactivate} />
        </TabsContent>
        <TabsContent value="subAdmins">
          <UserTable users={subAdmins} isLoading={isLoading} onEdit={(u) => { setUserToEdit(u); setOpen(true); }} onDeactivate={setUserToDeactivate} />
        </TabsContent>
        <TabsContent value="staff">
          <UserTable users={staff} isLoading={isLoading} onEdit={(u) => { setUserToEdit(u); setOpen(true); }} onDeactivate={setUserToDeactivate} />
        </TabsContent>
        <TabsContent value="all">
          <UserTable users={users ?? []} isLoading={isLoading} onEdit={(u) => { setUserToEdit(u); setOpen(true); }} onDeactivate={setUserToDeactivate} />
        </TabsContent>
      </Tabs>

      <AddEditUserDialog
        open={open}
        onOpenChange={(o) => { setOpen(o); if (!o) setUserToEdit(null); }}
        onSaved={() => mutate()}
        scope={user}
        departments={departments}
        subDepartments={subDepartments}
      ...
      <AlertDialog open={!!userToDeactivate} onOpenChange={(o) => { if (!o) setUserToDeactivate(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
