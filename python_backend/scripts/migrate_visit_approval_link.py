"""
Add one-time doctor approval link columns to Visit. Idempotent.

Usage:
  python scripts/migrate_visit_approval_link.py --yes
"""
...
MIGRATION_ID = "2026-06-26_visit_approval_link"
        ...
        if not dry_run and not already_applied:
            conn.execute(
                ...
                ),
                {
                    "id": MIGRATION_ID,
                    "appliedAt": now_ist(),
                    "notes": "One-time doctor approval link on Visit",
                },
            )
