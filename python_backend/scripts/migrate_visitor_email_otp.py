"""
Add email OTP columns to VisitorAccount. Idempotent.

Usage:
  python scripts/migrate_visitor_email_otp.py --dry-run
  python scripts/migrate_visitor_email_otp.py --yes
"""
...
MIGRATION_ID = "2026-06-06_visitor_email_otp"
...
def run(dry_run: bool) -> None:
    with engine.begin() as conn:
        ...
        for column, definition in COLUMNS:
            if column_exists(conn, "VisitorAccount", column):
                print(f"Skip VisitorAccount.{column} (exists)")
                continue
            sql = f"ALTER TABLE VisitorAccount ADD COLUMN {column} {definition}"
            print(sql)
            if not dry_run:
        ...
        if not dry_run and not already_applied:
            conn.execute(
                ...
                ),
                {
                    "id": MIGRATION_ID,
                    "appliedAt": now_ist(),
                    "notes": "Visitor account email OTP verification columns",
                },
            )
